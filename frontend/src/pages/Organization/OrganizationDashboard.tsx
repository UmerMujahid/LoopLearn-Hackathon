  ...
  return (
    <div className="space-y-8 animate-slide-up pb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          {
            icon: <FiSearch className="text-amber-600 dark:text-amber-400" size={24} />,
            label: 'Available Surplus Listings',
            value: statsLoading ? '...' : String(availableListings.length),
            sub: 'Fresh batches available for rescue',
            badgeBg: 'bg-amber-500/10 text-amber-800 dark:text-amber-300',
            borderColor: 'border-emerald-950 dark:border-emerald-800',
          },
          {
            icon: <FiPackage className="text-emerald-700 dark:text-emerald-400" size={24} />,
            label: 'My Active Claims',
            value: statsLoading ? '...' : String(totalRequests),
            sub: `${pendingRequests} pending provider approval`,
          ...
          },
          {
            ...
            label: 'Meals Successfully Rescued',
            value: statsLoading ? '...' : String(collectedFood),
            sub: 'Distributed to community members',
            badgeBg: 'bg-amber-500/10 text-amber-800 dark:text-amber-300',
            borderColor: 'border-emerald-950 dark:border-emerald-800',
          },
          {
            ...
            sub: 'Environmental footprint saved',
            badgeBg: 'bg-emerald-500/10 text-emerald-800 dark:text-emerald-300',
            borderColor: 'border-emerald-950 dark:border-emerald-800',
          },
        ].map((kpi, idx) => (
          <TiltCard key={idx} intensity={8}>
            <div className={`p-5 rounded-3xl bg-white dark:bg-[#0f1a14] border-2 ${kpi.borderColor} shadow-pop-sm flex flex-col justify-between h-full`}>
              <div className="flex items-center justify-between mb-3">
                <div className="w-10 h-10 rounded-2xl bg-amber-50 dark:bg-amber-950/60 border border-amber-900/20 flex items-center justify-center">
                  {kpi.icon}
                </div>
